...
local pl = getPlayer();
		...
		if getPlayer():getHoursSurvived()<=1 then
			...
			pl:setWornItem(toItemBodyLocation(holster:getBodyLocation()), holster);
			belt = inv:AddItem("Base.Belt2");
			pl:setWornItem(toItemBodyLocation(belt:getBodyLocation()), belt);
			...
			for i = 0 , pillowmod.injuryloops do 
				...
				-- burned
				if injury == 1 then 
					table.insert(damageOps, { op = "setBurnTime", partIndex = bodypart, time = pillowmod.avginjurytime })
					print("burning ");
				end --end burned

				--deepwound
				if injury == 2 then 
					table.insert(damageOps, { op = "setDeepWounded", partIndex = bodypart, enabled = true })
					table.insert(damageOps, { op = "setDeepWoundTime", partIndex = bodypart, time = pillowmod.avginjurytime })
					table.insert(damageOps, { op = "setBleedingTime", partIndex = bodypart, time = pillowmod.avgbleedtime })
					print("deep wound ");
					if injuryharshnesschance == 1 then 
						table.insert(damageOps, { op = "setHaveGlass", partIndex = bodypart, enabled = true })
				...
				if injury == 3 and pillowmod.fracturecount <= 2 then
					...
					table.insert(damageOps, {
						op = "setFractureTime",
						partIndex = bodypart,
						time = ZombRand(5, 20) + pillowmod.injurymodifier,
					})
					print("fracture");
				...
				if injury == 4 then 
					if  injuryharshnesschance == 2 then 
						table.insert(damageOps, { op = "SetScratchedWindow", partIndex = bodypart, enabled = true })
						table.insert(damageOps, { op = "setBleedingTime", partIndex = bodypart, time = pillowmod.avgbleedtime })
						print("scratch window ");
					else 
						print("cut weapon ");
						table.insert(damageOps, { op = "SetScratchedWeapon", partIndex = bodypart, enabled = true })
						table.insert(damageOps, { op = "setBleedingTime", partIndex = bodypart, time = pillowmod.avgbleedtime })
					end
				...
				if injury == 5 then --cut 
					print("cut  ");
					table.insert(damageOps, { op = "setCut", partIndex = bodypart, enabled = true })
					table.insert(damageOps, { op = "setCutTime", partIndex = bodypart, time = pillowmod.avginjurytime })
					table.insert(damageOps, { op = "setBleedingTime", partIndex = bodypart, time = pillowmod.avgbleedtime })
				end -- end cut

			end -- end dire loops

			applyBodyDamageOps(pl, damageOps)
